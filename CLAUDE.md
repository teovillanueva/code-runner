...
**Observability (telemetry-off by default):** Both API and worker are OpenTelemetry-instrumented but emit nothing until `OTEL_EXPORTER_OTLP_ENDPOINT` is set — a plain `docker compose up` is a true no-op. The example collector + Jaeger backend ships **inert** under the `observability` compose profile (mirrors the `stub` profile): `docker compose --profile observability up` + `bash scripts/observability-e2e.sh` brings it up and proves one connected trace (API `execute` span + worker phase spans, shared `trace_id`) in the Jaeger UI at http://localhost:16686. Collector config: `observability/otel-collector.yaml`. Lower `OTEL_TRACES_SAMPLER_ARG` in production. Redis/soketi self-instrumentation is documentation-only (operators add `redis_exporter` / soketi metrics themselves; no exporter is shipped).
