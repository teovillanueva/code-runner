...
[env]
  ...
  # ~2 jobs per machine; clamp to a sane ceiling.
  FAS_STARTED_MACHINE_COUNT = "min(20, max(1, ceil(qdepth / 2)))"
  # FAS_API_TOKEN / FAS_PROMETHEUS_TOKEN are set as secrets.
```

<Callout type="warn">
  fly-autoscaler queries **Prometheus**, so `code_runner_queue_depth` has to be in
  Fly's managed Prometheus — which means scraping it from a `/metrics` endpoint, not
  the OTLP push path that's off by default. See the
  [Autoscaling on Fly runbook](/docs/self-hosting/autoscaling-fly) for the full setup,
  including why **scheduled pre-warming** (you know when exams/batches run) beats
  reactive scaling for predictable bursts.
</Callout>
