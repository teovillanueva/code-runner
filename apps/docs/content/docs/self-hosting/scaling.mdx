...
```text
  Client / Upstream App
         ...
         ▼
  ┌──────────────┐
  │  API (Hono)  │  N replicas — stateless, scale freely
  └──────┬───────┘
         │  LPUSH jobs:queue
         ▼
  ┌──────────────┐
  │    Redis     │  native TCP (blocking ops required by the worker)
  └──────┬───────┘
         │  BRPOP / SUBSCRIBE stdin:<jobId>
         ▼
  ┌────────────────────────────────┐
  │  Worker NODE (Go, long-lived)  │  M replicas — scaled by queue depth
  │  ┌──────────┐  ┌──────────┐    │
