...
| `ARTIFACT_S3_PUBLIC_ENDPOINT` | _(unset)_ | Endpoint used to **sign** presigned GET URLs when clients fetch from a different host than the worker connects to (split-horizon). When set, the worker uploads via `AWS_ENDPOINT_URL_S3` but signs against this. See [Artifacts](/docs/guides/artifacts#browser-reachable-urls-split-horizon). |
