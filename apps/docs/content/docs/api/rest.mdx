...
| `files` | yes | Array of `FileInput`; at least one. File names 1–255 chars. See [file inputs](#file-inputs). |
...
| `ref` | conditional | `"sha256:<64-hex>"` — a pre-uploaded [content-addressed blob](#blobs). Mutually exclusive with `content`. |
...
of the API (host-escape-only threat model). See [Multi-file input](/docs/guides/input-files)
and [Large & shared files](/docs/guides/blobs).
...
[Blobs guide](/docs/guides/blobs) for the full flow, dedup, and TTL/GC behaviour.
...
<Callout type="info">
  ...
  **501** and you should keep using inline files. See
  [Configuration](/docs/self-hosting/configuration#object-storage--artifacts).
</Callout>

### POST /v1/blobs/check

Ask which of a set of blobs the store already has. Missing blobs come back with a
presigned PUT URL; present blobs have their idle TTL refreshed.

**Request body**

```json
{ "hashes": ["sha256:9f86d0…<64 hex>", "sha256:2c26b4…<64 hex>"] }
```
